...
self.addZone = async function(zone, subnet) {
	if(zone && subnet) {
		console.log("[INFO]: bind [zones.create] zones [" + zone + ":" + subnet + "]");
...
self.delZone = async function(zone) {
	if(zone) {
		console.log("[INFO]: bind [zones.delete] zones [" + zone + "]");
...
self.createZone = async function(zoneName, fileName, fileData) {
	...
	if(data && data[zoneName]) {
		console.log("[INFO]: bind [zones.create] zone [" + zoneName + "] exists!");
	} else {
		console.log("[INFO]: bind [zones.create] zone [" + zoneName + "] does not exist!");
		await file.write(fileName, fileData);
		await rndc.add(zoneName, fileName)
		console.log("[INFO]: bind [zones.create] zone [" + zoneName + "] created");
...
self.deleteZone = async function(zoneName) {
	let data = await zonedb.load()
	if(data && data[zoneName]) {
		console.log("[INFO]: bind [zones.delete] zone [" + zoneName + "] exists!");
		await rndc.del(zoneName);
		console.log("[INFO]: bind [zones.delete] zone [" + zoneName + "] deleted");
		await zonedb.refresh();
	} else {
		console.log("[INFO]: bind [zones.delete] zone [" + zoneName + "] does not exist!");
	}
	return zoneName;
}
